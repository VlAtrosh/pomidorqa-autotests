...
test.describe('API: бронирование слота PomidorQA', () => {
  ...
  test('можно отменить бронь раньше чем за 2 часа — 200, слот снова free', async () => {
    const slot = store.createSlot('user-host-cancel-ok', futureIso(180)); // 3 часа

    const bookingResponse = await api.post('/bookings', {
      data: { slotId: slot.id, userId: 'user-guest-cancel-ok' },
    });
    const booking = await bookingResponse.json();
    expect(booking.status).toBe('confirmed');

    const cancelResponse = await api.delete(`/bookings/${booking.id}`);
  ...
  test('нельзя отменить бронь позже чем за 2 часа — 409 cancel_too_late', async () => {
    const slot = store.createSlot('user-host-cancel-late', futureIso(60)); // 1 час

    const bookingResponse = await api.post('/bookings', {
      data: { slotId: slot.id, userId: 'user-guest-cancel-late' },
    });
    const booking = await bookingResponse.json();
    ...
    expect((await cancelResponse.json()).error).toBe('cancel_too_late');
  });
});
